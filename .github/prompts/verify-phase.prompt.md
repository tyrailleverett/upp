...
1. Parse `${input:arguments}` — expect two space-separated values: the plan file path and the PR URL or number
...
5. Abort with a clear error if:
   - `${input:arguments}` is missing or does not contain two values
