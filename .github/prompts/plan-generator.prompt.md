...
0. **Read the PRD** — If a PRD path was provided via `${input:prdPath}` (optional — leave blank to use default), read the file at that path. If no path was provided, check if `specs/prd.md` exists and read it. If a PRD is found, extract: Product Overview, Core Entities, MVP Feature Set, Data Model, and Non-Functional Requirements. Use these as the primary input for phase decomposition — the PRD defines *what* to build, the plan defines *how* to build it.
