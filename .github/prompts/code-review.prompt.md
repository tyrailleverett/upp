...
Use `#tool:get_changed_files` to get the list of staged and unstaged modified files. Filter to PHP files, Blade templates, and TypeScript/React files (`.ts`, `.tsx`). Skip `vendor/`, `node_modules/`, lock files, and generated stubs.
...
For any finding in **Category 4** (architecture) or **Category 6** (application conventions) where you are not certain a pattern is incorrect, use `#tool:search-docs` to verify against the Laravel documentation **before** including it as a finding. Only flag something after confirmation. Do not flag uncertain API usage without checking.
